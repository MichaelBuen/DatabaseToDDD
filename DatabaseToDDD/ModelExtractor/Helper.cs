...
namespace ModelExtractor
{
    ...
    public static class TheModelExtractor
    {
        ...
        static string Right(this string str, int length)
        {
            return str.Substring(str.Length - length, length);
        }
        

        public static string NormalizeNavigationName(string columnName)
        {
            string propertyName =
               columnName.Right(4).Equals("Code", System.StringComparison.CurrentCultureIgnoreCase) ?
                   columnName.Substring(0, columnName.Length - 4) :
               columnName.Right(2).Equals("ID", System.StringComparison.CurrentCultureIgnoreCase) ?
        ...
        public static IList<Table> GetTables()
        {
            ...
            {
                return db.SqlList<Table>(Constants.TableQuery);
            }
        }

        public static IList<Column> GetColumns()
        {
            var dbFactory = new OrmLiteConnectionFactory(Constants.ConnectionString, SqlServerDialect.Provider);

            using (var db = dbFactory.OpenDbConnection())
            {
                return db.SqlList<Column>(Constants.ColumnQuery);
            }
        }

        public static IList<EntityAssociation> GetEntityAssociation()
        {
            var dbFactory = new OrmLiteConnectionFactory(Constants.ConnectionString, SqlServerDialect.Provider);

            using (var db = dbFactory.OpenDbConnection())
            {
                return db.SqlList<EntityAssociation>(Constants.EntityAssociationQuery);
            }
        }
    }
...
";
        ...
        public const string EntityAssociationQuery =
@"with a as
(
